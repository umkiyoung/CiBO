...
from baselines.utils import get_lagrangian
...
class ProxyEnsemble(nn.Module):
    def __init__(self, x_dim, hidden_dim=1024, num_hidden_layers=1, output_dim=1, n_ensembles=5, ucb_reward=False, indicator = 'false', constraint_formulation="Lagrangian", lamb=1.0):
    ...
    def log_reward(self, x, beta=1.0):
        if self.ucb_reward:
            ...
            if self.constraint_formulation == "Soft":
                ...
                # pred_c = self.soft_constraint(constrain_stacked, self.lamb)
                # pred_y = pred_y + pred_c
                assert False
                
            elif self.constraint_formulation == "LogBarrier":
                # # pred_c = self.log_barrier_constraint(mean[:, 1:], self.lamb)
                # pred_c = self.log_barrier_constraint(constrain_stacked, self.lamb)
                # pred_y = pred_y + pred_c
                assert False
                ...
                #     pred_c = self.lagrangian_constraint(constrain_stacked, self.lamb)
                # else:
                #     # pred_c = self.lagrangian_indicator_constraint(mean[:, 1:], self.lamb)
                #     pred_c = self.lagrangian_indicator_constraint(constrain_stacked, self.lamb)
                # pred_y = (1-self.lamb) * pred_y + pred_c
    ...
    #     return - lamb * torch.sum(mean.clamp(min=0), dim=-1) + 1e-08
    
    # def lagrangian_indicator_constraint(self, mean, lamb):
    #     return - lamb * torch.sum(mean.clamp(min=0,max=1), dim=-1) + 1e-08
